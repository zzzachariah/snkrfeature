    ...
    <main className="container-shell py-8">
      <form ref={formRef} onSubmit={onSubmit} className="surface-card premium-border mx-auto grid max-w-5xl gap-4 rounded-2xl p-6 md:grid-cols-2">
        <div className="md:col-span-2 space-y-2">
          <h1 className="text-2xl font-semibold tracking-[0.015em]">Submit sneaker information</h1>
          <p className="text-sm soft-text">Submissions are stored as raw payload, normalized server-side, and routed to admin review before publication.</p>
        </div>

        {fields.map(([name, label, required]) => (
          <div key={name}>
            <label className="mb-1 block text-xs soft-text">{label}</label>
            <Input name={name} placeholder={label} required={required} type={name === "release_year" ? "number" : "text"} />
          </div>
        ))}

        <div className="md:col-span-2">
          <label className="mb-1 block text-xs soft-text">Story / background notes</label>
          <textarea name="story_notes" className="min-h-24 w-full rounded-xl border border-[rgb(var(--muted)/0.55)] bg-[rgb(var(--bg-elev)/0.78)] p-3 text-sm text-[rgb(var(--text))] outline-none transition focus:border-[rgb(var(--ring)/0.8)] focus:ring-4 focus:ring-[rgb(var(--ring)/0.16)]" placeholder="Release context, design intent, notable versions, community notes." />
        </div>

        <div className="md:col-span-2">
          <label className="mb-1 block text-xs soft-text">Raw notes (required)</label>
          <textarea name="raw_text" className="min-h-40 w-full rounded-xl border border-[rgb(var(--muted)/0.55)] bg-[rgb(var(--bg-elev)/0.78)] p-3 text-sm text-[rgb(var(--text))] outline-none transition focus:border-[rgb(var(--ring)/0.8)] focus:ring-4 focus:ring-[rgb(var(--ring)/0.16)]" placeholder="Paste your full performance observations and source snippets..." required />
        </div>
